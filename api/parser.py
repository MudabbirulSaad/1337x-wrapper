...
def torrentParser(response, baseUrl, page=1, torrent_only=False):
    ...
    if torrent_only:
        ...
        if torrentList:
            for count, torrent in enumerate(torrentList):
                ...
                seeders = int(seedersList[count].getText())
                leechers = int(leechersList[count].getText())
        ...
        if torrentList:
            for count, torrent in enumerate(torrentList):
                name = torrent.getText().strip()
                torrentId = int(torrent['href'].split('/')[2])
